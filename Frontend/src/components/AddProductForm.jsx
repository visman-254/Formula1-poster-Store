...
import { Camera, Palette, Upload } from "lucide-react";
...
const AddProductForm = () => {
  ...
  const [quickStorageRam, setQuickStorageRam] = useState("");
  ...
  const [colorImageMapping, setColorImageMapping] = useState([]);
  ...
  const handleQuickGenerate = () => {
    ...
    const storageRamPairs = quickStorageRam.split(',').map(pair => {
      const parts = pair.split('-').map(p => p.trim());
      return { storage: parts[0] || '', ram: parts[1] || '' };
    }).filter(p => p.storage || p.ram);
    ...
    if (allColors.length === 0 && storageRamPairs.length === 0) {
      setMessage("Please enter at least one color and one Storage-RAM pair");
      return;
    }
    ...
    const storageRam = storageRamPairs.length > 0 ? storageRamPairs : [{ storage: '', ram: '' }];
    const stocks = stockList.length > 0 ? stockList : ['1'];

    colors.forEach((color, colorIndex) => {
      storageRam.forEach((sr) => {
        ...
        if (colorImageMapping[colorIndex]) {
          const mappedImage = colorImageMapping[colorIndex];
          variantImage = mappedImage.file;
          variantImagePreview = mappedImage.preview;
        } else if (quickImages.length > 0) {
          const imgIndex = variantIndex % quickImages.length;
          variantImage = quickImages[imgIndex].file;
          variantImagePreview = quickImages[imgIndex].preview;
        }
        ...
        generatedVariants.push({
          ...
          profit_margin: (sellingPrice - buyingPrice).toString(),
          discount: '0',
  ...
  const handleBundleItemQuantityChange = (variant_id, quantity) => {
    setBundleItems(bundleItems.map(item =>
      item.variant_id === variant_id ? { ...item, quantity: parseInt(quantity) || 1 } : item
    ));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isBundle) {
        ...
        // Append each image with field name "images" (plural)
  ...
  return (
    <Card className="min-h-screen bg-glass p-4">
      ...
      <CardContent>
        {message && (
          <p className={`mb-4 text-sm font-semibold ${message.startsWith("❌") ? "text-red-600" : "text-green-600"}`}>
            {message}
          </p>
        )}
                ...
                <div className="mb-6 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                  ...
                  {useQuickGenerate && (
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                      ...
                      <div className="space-y-2">
                        <Label>Storage-RAM Pairs (comma-separated)</Label>
                        <Input
                          value={quickStorageRam}
                          onChange={(e) => setQuickStorageRam(e.target.value)}
                          placeholder="128GB-4GB, 256GB-8GB, 512GB-12GB"
                        />
                        <p className="text-xs text-gray-500">Format: Storage-RAM, e.g., 256GB-8GB</p>
                      </div>
                      ...
                      <div className="col-span-2 md:col-span-4 space-y-3">
                        <div>
                          <Label>Upload Images by Color</Label>
                          <p className="text-xs text-gray-500">One image per color - applies to all storage variants</p>
                        </div>

                        {(() => {
                          const colorList = savedColors.length > 0
                            ? savedColors
                            : quickColors.split(',').map((c, i) => ({ hex: c.trim(), name: c.trim() || `Color ${i + 1}` })).filter(c => c.hex);

                          if (colorList.length === 0) {
                            return (
                              <div className="text-sm text-gray-400 italic p-4 border rounded">
                                Add colors above to see image slots
                              </div>
                            );
                          }
                              ...
                              {colorList.map((color, colorIdx) => (
                                ...
                                >
                                  ...
                                  <input
                                    ...
                                    accept="image/*"
                                    className="hidden"
                                    id={`color-img-${colorIdx}`}
                                    name="images"  // Added name attribute
                                    onChange={(e) => {
                                      const file = e.target.files[0];
                                      if (file) {
                                        const newColorImages = [...(colorImageMapping || [])];
                                        newColorImages[colorIdx] = {
                                          file,
                                          preview: URL.createObjectURL(file)
                                        };
                                        setColorImageMapping(newColorImages);
                                      }
                                    }}
                                  />
                                  <label
                                    htmlFor={`color-img-${colorIdx}`}
                                    className="cursor-pointer block"
                                  >
                                    {colorImageMapping && colorImageMapping[colorIdx] ? (
                                      <div className="relative">
                                        <img
                                          src={colorImageMapping[colorIdx].preview}
                                          alt={color.name}
                                      ...
                                      <div className="w-full h-20 border-2 border-dashed border-gray-300 rounded flex items-center justify-center hover:border-blue-400">
                      ...
                      {colorImageMapping.filter(Boolean).length > 0 && (
                        <div className="mt-2 p-2 bg-green-50 dark:bg-green-900/20 rounded">
                          <p className="text-xs text-green-700 dark:text-green-300">
                            ✓ {colorImageMapping.filter(Boolean).length} colors with images mapped
                          </p>
                        </div>
                      )}
                      ...
                      <div className="col-span-2 md:col-span-4 flex gap-2 mt-2">
                        <Button
                          ...
                          className="bg-white/10 backdrop-blur-md hover:bg-white/20 border border-white/20 text-gray-700 hover:text-gray-900 dark:border-white/20 dark:text-gray-300 dark:hover:text-white transition-all duration-300 shadow-lg hover:shadow-xl"
                        >
                          Generate {((([...quickColors.split(',').filter(c => c.trim()), ...savedColors.map(c => c.hex)].filter(c => c).length) || 1) * (quickStorageRam.split(',').filter(p => p.trim()).length || 1))} Variants
                        </Button>
                        <Button
                          type="button"
                          variant="outline"
