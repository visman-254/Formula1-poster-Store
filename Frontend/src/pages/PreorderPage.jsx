...
import "./Products.css";
import PreorderModal from "../components/PreorderModal"; // Will be created later
...
const PreorderPage = () => {
  ...
  const [selectedProductForPreorder, setSelectedProductForPreorder] = useState(null);
  ...
  useEffect(() => {
    const fetchProducts = () => {
      fetch(`${API_BASE}/api/products`)
        .then((res) => res.json())
        .then((data) => {
          const preorderableProducts = data.filter(p => p.is_preorderable);
          setProducts(preorderableProducts);
          setFilteredProducts(preorderableProducts);
          
          const initialVariants = {};
          preorderableProducts.forEach(product => {
  ...
  const handlePreorder = (product) => {
    ...
    setSelectedProductForPreorder({ ...product, selectedVariant });
    setIsPreorderModalOpen(true);
  };
  
  const handleCloseModal = () => {
    setIsPreorderModalOpen(false);
    setSelectedProductForPreorder(null);
  }
        ...
        <div className="grid flex-grow grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 gap-3 p-4 md:p-8">
          {filteredProducts.length > 0 ? (
            filteredProducts.map((product) => {
              ...
              const originalPrice =
                (Number(selectedVariant.price) || 0) + (Number(selectedVariant.discount) || 0);
              ...
              <div className="max-w-md mx-auto">
                ...
                <p className="text-gray-600 dark:text-gray-300">
                  {searchQuery
                    ...
                    : "No products available for preorder at the moment."}
                </p>
                {searchQuery && (
                  <Button
                    variant="outline"
...
const DescriptionText = ({ description }) => {
  ...
  const isShort = description.split(" ").length <= 3;
